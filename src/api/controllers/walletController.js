...
import { generateAddressBuffer } from "../utils/helpers"
...
const availableTokens = [
  ...
  },
  {
    ...
    name: "Sepolia",
    iconUrl:
      "https://metaschool.so/_next/static/media/unknown-logo.7eda54b1.webp",
    address: "",
  },
  {
    id: "USDT",
...
async function token_breakdown_against_wallet(req, res) {
  ...
  try {
    ...
    const filteredErc20Tokens = availableTokens.filter((token) =>
      erc20TokensToFilter.includes(token.id)
    )

    // Step 2: Collect balances for ERC20 tokens
    const erc20TokenBalances = await alchemyInstance.core.getTokenBalances(
      walletId,
      filteredErc20Tokens.map((t) => t.address)
    )
    ...
    const networkBalances = await Promise.all(
      networks.map(async ({ network, key, name }) => {
        const balance = await getAlchemy(network).core.getBalance(
          walletId,
          "latest"
        )
    ...
    if (erc20TokenBalances && erc20TokenBalances.tokenBalances.length > 0) {
      erc20TokenBalances.tokenBalances.forEach((tokenBalanceRes) => {
        const matchedErc20Token = filteredErc20Tokens.find(
          (token) => token.address === tokenBalanceRes.contractAddress
        )
        if (matchedErc20Token) {
          const resObject = {
            key: matchedErc20Token.id,
            network: {
              iconUrl: matchedErc20Token.iconUrl,
              name: matchedErc20Token.name,
              symbol: matchedErc20Token.symbol,
            },
    ...
    res.status(500).json({
      success: false,
      message: `Error in tokenBreakdownAgainstWallet: ${err.message}`,
    })
  }
}
